...
cnt = 0
logfile = open('c:\\fullfilelist.txt', 'w+')
dirs = [u"d:\\"]
for a_dir in dirs:
    for (path, dir, files) in os.walk(a_dir):
        for filename in files:
            cnt += 1
            if cnt % 10000 == 0:
                print cnt
            fullname = path + '\\' + filename
            #fullname = path.decode('cp949') + filename.decode('cp949')
            #print fullname
            try:
                filesize = os.path.getsize(fullname)
                infile = open(fullname, 'r')
                md5hash = md5.new(infile.read(4096)).hexdigest()
            except:
                filesize = 0
                md5hash = ''
            #print filesize
            #print md5hash
            logfile.write(fullname + '\t' + str(filesize) + '\t' + md5hash + '\n')        
